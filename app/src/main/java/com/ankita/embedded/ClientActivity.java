...
public class ClientActivity extends AppCompatActivity implements View.OnClickListener{
  ...
  //  public static String SERVER_IP;
    ...
    public String SERVER_IP = intent.getStringExtra("ipaddress");

   // public String SERVER_IP = getIntent().getStringExtra("ipaddress");
  //  public Integer SERVER_PORT = getIntent().getIntExtra("portnumber", 8080);
                ...
                if(isConnected && out!=null){
                    switch(event.getAction()){
                        ...
                        case MotionEvent.ACTION_UP:
                            ...
                            if(!mouseMoved){
                                out.println(Control.MOUSE_LEFT_CLICK);
                            }
                    }
                }
                return true;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.playPauseButton:
                if (isConnected && out!=null) {
                    out.println(Control.PLAY);//send "play" to server
                }
                break;
            case R.id.nextButton:
                if (isConnected && out!=null) {
                    out.println(Control.NEXT); //send "next" to server
                }
                break;
            case R.id.previousButton:
                if (isConnected && out!=null) {
                    out.println(Control.PREVIOUS); //send "previous" to server
                }
                break;
        }
